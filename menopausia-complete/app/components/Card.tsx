      ...
      >
        ...
        <div className="flex-1 relative bg-zinc-800 overflow-hidden">
          <img
            ...
            className="w-full h-full object-contain bg-zinc-950 group-hover:scale-105 transition-transform duration-700"
          />
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isFullyLocked
          ? 'border-zinc-800 bg-zinc-900 opacity-100 hover:border-brand-500/50' // Removed opacity-70
    ...
    >
      {/* Locked Overlay */}
      {isFullyLocked && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/40 backdrop-blur-[1px] transition-all group-hover:backdrop-blur-none group-hover:bg-black/30">
      ...
      <div className="flex-1 relative overflow-hidden bg-zinc-800">
        <img
          ...
          alt={product.title}
          onError={(e) => { e.currentTarget.src = "https://images.unsplash.com/photo-1504052434569-70ad5836ab65?q=80&w=2070"; }}
          className={`w-full h-full object-contain bg-zinc-950 transition-transform duration-500 ${isFullyLocked ? 'opacity-90' : 'group-hover:scale-105'}`}
        />
        {!isFullyLocked && (
          <div className="absolute top-2 right-2 px-2 py-1 bg-black/70 backdrop-blur-md rounded text-xs font-medium text-white border border-white/10 uppercase">
            {product.category === 'BONUS' ? t('modal.bonus') : product.category === 'MAIN' ? t('sections.mainCollection') : t('modal.file')}
          </div>
        )}
      ...
      <div className="p-4 bg-zinc-950 border-t border-zinc-800 relative z-10">
        ...
        <div className="mt-2 pt-2 border-t border-zinc-800 flex items-center justify-between">
          {product.category === 'UPSELL' && isFullyLocked ? (
            <span className="text-brand-500 font-bold text-sm">US$ {product.price?.toFixed(2)}</span>
          ) : isFullyLocked ? (
