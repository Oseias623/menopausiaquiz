                            ...
                            <form onSubmit={handleSubmit} className="space-y-8 bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <select name="category" value={formData.category} onChange={handleInputChange} className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-4 py-2 text-white outline-none focus:border-brand-500">
                                                <option value="LETTER">Livro Principal (Módulos)</option>
                                                <option value="BONUS">Bônus (Material Extra)</option>
                                                <option value="UPSELL">Upsell (Oferta Externa)</option>
                                                {/* LAYOUT is hidden from normal selection */}
                                                {formData.category === 'LAYOUT' && <option value="LAYOUT">Configuração de Layout</option>}
                                            </select>
                                        </div>
                                        <div>
                                            ...
                                            {formData.category === 'UPSELL' || formData.category === 'LAYOUT' ? (
                                                <div className="w-full bg-zinc-800/50 border border-zinc-700 rounded-lg px-4 py-2 text-zinc-400 text-sm flex items-center gap-2 cursor-not-allowed">
                                                    ...
                                                    <span>Venda Avulsa (Hotmart)</span>
                                                </div>
                                            ) : (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <MediaInput
                                            label={formData.category === 'UPSELL' ? "Link de Checkout (Hotmart)" : "Arquivo PDF (Geral)"}
                                            icon={formData.category === 'UPSELL' ? ShoppingBag : FileText}
                                            mode={formData.category === 'UPSELL' ? 'LINK' : pdfMode}
                                            setMode={formData.category === 'UPSELL' ? () => { } : setPdfMode}
                                            file={pdfFile} setFile={setPdfFile}
                                            link={pdfLink} setLink={setPdfLink}
                                            accept="application/pdf"
                                            fixedMode={formData.category === 'UPSELL' ? 'LINK' : undefined}
                                        />
                                        <MediaInput
...
const MediaInput = ({ label, icon: Icon, mode, setMode, file, setFile, link, setLink, accept, fixedMode }: any) => {
    return (
        <div className="border border-zinc-800 bg-zinc-900 rounded-xl overflow-hidden flex flex-col h-full">
            <div className="bg-zinc-950 px-3 py-2 border-b border-zinc-800 flex items-center justify-between">
                ...
                <div className="flex bg-zinc-900 rounded border border-zinc-800 p-0.5">
                    {/* If fixedMode is set, only show the relevant button (or disable toggling) */}
                    {/* Simple solution: Hide buttons if fixedMode is set, just show active icon */}

                    {(!accept || accept !== 'application/pdf' || !fixedMode) && (
                        <button
                            type="button"
                            onClick={() => !fixedMode && setMode('FILE')}
                            className={`p-1 rounded ${mode === 'FILE' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'} ${fixedMode ? 'opacity-50 cursor-not-allowed' : ''}`}
                    ...
                    <button
                        type="button"
                        onClick={() => !fixedMode && setMode('LINK')}
                        className={`p-1 rounded ${mode === 'LINK' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'} ${fixedMode ? 'opacity-50 cursor-not-allowed' : ''}`}
                        title="Link Externo"
                        disabled={!!fixedMode}
                    >
